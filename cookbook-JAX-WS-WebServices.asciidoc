...
Furthermore we have to use the _publish_ method of our `org.apache.cxf.jaxws.EndpointImpl` to define the last part of our WebService-URI.

If you now fire up the sample application with link:getting-started-running-sample-application#step-2-run-the-application[SpringBoot], opening a browser and hit below URL where the webservice is hosted
