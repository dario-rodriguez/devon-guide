...
It is mandatory to follow the link:Contributing-Code-of-Conduct[code of conduct] that must be present in the root of every OSS or private project as `CODE_OF_CONDUCT.asciidoc` or `CODE_OF_CONDUCT.md`. 
...
* Please read the link:Contributing-Git-Fork-guide[Working with forked repositories] document to learn all about this topic.
