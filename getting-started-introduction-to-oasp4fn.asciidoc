...
* Functions in FaaS are **triggered by the event types** defined by the provider. 
For example:
** HTTP requests.
** AWS S3 updates.
...
It solves common and inefficient situations such as **occasional requests**, where an application with a few small requests per minute makes the CPU idle most of time, or like **inconsistent traffic** where the traffic profile of an application is very _speaky_. For example:
