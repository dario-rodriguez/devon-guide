...
Open the Devonfw console by executing the batch file "console.bat" from the Devon distribution. This will open a command prompt preconfigured automatically to use the software and configuration provided by the Devonfw distribution.
...
+
* WAR packaging (arguments before archetype:generate identify the Devon archetype):
+
...
---- 
+
* And for combined WAR and EAR packaging (arguments before archetype:generate identify the Devon archetype):
+
[source]
---- 
mvn -DarchetypeVersion=2.0.0 -DarchetypeGroupId=io.oasp.java.templates -DarchetypeArtifactId=oasp4j-template-server archetype:generate -DgroupId=io.oasp.application -DartifactId=sampleapp -Dversion=0.1-SNAPSHOT -Dpackage=io.oasp.application.sampleapp 
...
Now you need to fill the Group Id, Artifact Id, Version and Package for your project. If you want to add an EAR generation mechanism to your project, you should fill the property earProjectName with the value Artifact Id + "-ear". For example, "sampleapp-ear". If you only want to WAR generation, you can remove the property earProjectName.
...
You can read more about the structure of a OASP4J application [here].