...
Then, in the _listener_ application, after adding the link:cookbook-integration-module#adding-the-module-to-a-project[module dependency] and the link:cookbook-integration-module#injecting-the-module[injection] of it, we only need to use the _subscribeTo_ method and provide the name for the channel and the queue and the _Message Handler_ to manage the received messages. For more details about the _Message Handler_ check out the link:cookbook-integration-module#listener-application-example[listener application example] section.
