...
[source]
----
integration.request-reply.emitter=false
integration.request-reply.listener=false
integration.request-reply.channelname=rr.Channel
integration.request-reply.queuename=rr.queue
integration.request-reply.receivetimeout=5000
----
...
Optionally, you can edit the name for the channel and for the queue using the `integration.request-reply.channelname` and `integration.request-reply.queuename` properties. As we just mentioned, the timeout for the response can be edited adding the `integration.request-reply.receivetimeout` property to our properties file and providing a milliseconds value. By default the timeout is _5000_ (5 seconds). 
...
You can also edit the name for both the _channel_ and the _queue_ (`integration.request-reply.channelname` and `integration.request-reply.queuename` properties). But, as mentioned in previous section, have into account that *these names must match* between the _sernder-receiver_ and the _receiver-sender_ applications, in order to perform the communication.
...
public class MySecondApp{
  ...
  public void myMethod(){
    ...
    this.integration.subscribeAndReply(new SimpleIntegrationHandler());
  }
}
----

In this case we provide to the `subscribeAndReply` method an _Integration Handler_ to manage the responses to each message. For the example we have implemented a basic Integration handler, to create your own one you only need to create a class, annotate it with `@Handler` and implement the `IntegrationHandler` interface. Lets see our _SimpleMessageHandler_
...
[source]
----
integration.request-reply-async.emitter=false
integration.request-reply-async.listener=false
integration.request-reply-async.channelname=async.Channel
integration.request-reply-async.queuename=async.queue
integration.request-reply-async.receivetimeout=5000
----
...
In the configuration of this application we can also define the timeout for the response. If exceeded, the process will be stopped and a _timeout exception_ will be thrown. This can be controlled with the property `integration.request-reply-async.receivetimeout`
...
public class MyListenerApp{
  ...
  public void readSimpleMessage(){
    ...
    this.integration.subscribeTo("my-channel", "my-queue", new SimpleMessageHandler());
  }
}
----

By default, the interval for polling the channel is _5000_ (5 seconds) and can be changed through property `integration.default.poller.rate` in `application.properties` file. In addition, you can define that value when creating the channel passing the milliseconds timeout as a parameter

[source,java]
----
this.integration.subscribeTo("my-channel", "my-queue", new SimpleMessageHandler() 10000);
----
...
public class MyFirstApp{
  ...
  public void startCommunication(){
    ...
    IntegrationChannel myChannel = this.integration.createRequestReplyChannel("my-channel", "my-queue", new MyMessageHandler());
    Boolean sent = myChannel.send("Hello");
    if (sent) System.out.println("message successfully sent");
  }
}
...
public class MySecondApp{
  ...
  public void startCommunication(){
    ...
    this.integration.subscribeAndReplyTo("my-channel", "my-queue", new MyIntegrationHandler());
  }
}
----
...
To create that type of channels the implementation is exactly the same than the link:cookbook-integration-module#creating-and-using-a-new-request-reply-channel[previous section]. So in this section we are going to show only the code differences
...
IntegrationChannel demoAsyncChannel = 
    this.integration.createAsyncRequestReplyChannel("my-async-channel", "my-async-queue", new MyMessageHandler());
----

You can define your own values for the _ThreadPoolExecutor's core pool size_ and _response timeout_ adding the properties `integration.default.poolsize` and `integration.default.receivetimeout` to your `application.properties` file and providing a value.
...
----
IntegrationChannel demoAsyncChannel = 
  this.integration.createAsyncRequestReplyChannel("my-async-channel", "my-async-queue", new MyMessageHandler(), 15, 10000);
----

And the second app you can subscribe to the channel with the method _subscribeAndReplyAsyncTo_ and providing the names for the channel and queue (that *must match* with the names provided in the first application), and an _IntegrationHandler_ to manage the messages and provide a reply.

[source,java]
----
this.integration.subscribeAndReplyAsyncTo("my-async-channel", "my-async-queue", new MyIntegrationHandler());
----

The implementation for the _Integration Handler_ is explained link:cookbook-integration-module#receiver-sender-application-example[here], except that we don't need to annotate the class with `@Handler`.

=== Sending headers with the message

The _Integration module_ also allows to send headers alongside the message content. To do so you can use the methods provided by the module that accespt a _Map_ as parameter for headers.
