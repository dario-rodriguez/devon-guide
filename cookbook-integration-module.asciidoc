...
After you have added the link:cookbook-integration-module#adding-the-module-to-a-project[module dependency] you can start using it link:cookbook-integration-module#injecting-the-module[injecting it]. Lets see how to send a simple message through that default _simple channel_.
...
With the _Integration module_ a _request-reply_ is provided by default. This channel will allow us to communicate systems sending and receiving messages in both sides. A first application will send a message and wait for a response, while a second application will receive the message sent by the first one and send back the response that the other app is expecting.
...
To enable the sending of messages through this channel you must set the _request-reply.emitter_ property to _true_ in the `application.properties` of our project.
...
After you have added the link:cookbook-integration-module#adding-the-module-to-a-project[module dependency] you can start using it link:cookbook-integration-module#injecting-the-module[injecting it]. Lets see how to send and receive a simple message through that default _request-reply channel_.
...
In this application we need to enable the _request-reply.listener_ property so, in the `applications.property` file of our project, we must set to _true_ that property.
...
As you can see in the above image the channel and queue for our request-reply channel have been created automatically and in the _Number of Consumers_ you can see that _1_ that refers to our application.
...
We have finished the demonstration for the default Request-Reply channel provided by the _Integration module_. Now we are going to see how to accomplish the same but in an asynchronous way using the third default channel provided by the module: the _request-reply-async_ channel.
