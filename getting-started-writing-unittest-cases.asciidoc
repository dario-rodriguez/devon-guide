...
TODO -> Add the image into the repository
...
Now, that we know how to create, write and run a test, we can start understanding the last steps. Let's to see how to work with TDD with a little tutorial.

=== Step 1: Write a test

The idea is very simple, we are going to create first the tests for the classes that we are going to implement after.
