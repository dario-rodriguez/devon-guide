...
[NOTE]
=====
Clearly, in the methods `add()` and `sub()`,  the `Object` is returned, because if the methods return an `int` now, we get a "red error" of `NullPointerException` instead of "blue error" of `assetThat()`. It's just for this tutorial.
=====
