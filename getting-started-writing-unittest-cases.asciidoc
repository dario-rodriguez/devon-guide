...
Create a class with the name _MyClass.java_ (you can link:getting-started-creating-new-devonfw-oasp4j-application[create a new application] as per need). In Eclipse, right click on the package in the application and then select _New > Class_. Name it _MyClass_ and press _Finish_.
