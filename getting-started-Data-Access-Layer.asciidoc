...
For example if one wanted to extend the example application so that all ingredients of one _FoodDrink_ can be saved and to model the ingredients themselves as entities (e.g. to store additional information about them), this could be modeled as follows (extract of a _FoodDrink_ class):
[source,java]
----
