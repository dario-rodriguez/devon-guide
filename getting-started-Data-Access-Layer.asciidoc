...
As a best practice we are using optimistic locking for regular end-user link:getting-started-Creating-Rest-Service[services] (OLTP) and pessimistic locking for link:cookbook-batch-layer[batches].
...
==== Pessimistic Locking
For back-end link:getting-started-Creating-Rest-Service[services] and especially for link:cookbook-batch-layer[batches] optimistic locking is not suitable. A human user shall not cause a large batch process to fail because he was editing the same entity. Therefore such use-cases use pessimistic locking what gives them a kind of priority over the human users.
...
=== Database Auditing
See link:getting-started-logging-and-auditing#auditing-with-hibernate-envers[auditing guide].
...
The link:getting-started-understanding-oasp4j-spring-boot-config[configuration] for Spring and Hibernate is already provided by OASP4J in our sample application and the application template. So you only need to worry about a few things to customize.
...
You can see more examples of database configurations link:getting-started-database-configuration[here]
...
=== SQL injection
A common link:cookbook-security-layer[security] threat is http://en.wikipedia.org/wiki/SQL_injection[SQL-injection]. Never build queries with string concatenation or your code might be vulnerable as in the following example:
