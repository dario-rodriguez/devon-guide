...
In the earlier chapter about CRUD functionality you saw the individual steps necessary to implement a basic CRUD functionality.

Using Cobigen, you can save most of the steps, and get a working result in far less time.
...
image::images/devon_guide_cobigen_1_context_menu.png[,scaledwidth=80%]

This action opens a code generator wizard, like this:

image::images/devon_guide_cobigen_2_template_selection.png[,scaledwidth=80%]
...
image::images/devon_guide_cobigen_3_field_selection.png[,scaledwidth=80%]
...
image::images/devon_guide_cobigen_4_warnings.png[,scaledwidth=80%]
...
Sometimes we need to include roles and permissions to our applications. CobiGen has a template to do this task. If you want to use this template follow the next steps:

=== Step 1: Define the permissions and roles

Open /oasp4j-sample-core/src/main/resources/config/app/security/access-control-schema.xml and define the permissions to the roles or group of roles like:
...
Right click on /oasp4j-sample-core/src/main/resources/config/app/security/access-control-schema.xml and select Cobigen > Generate... 
   
[NOTE]
====
It is possible that you will have problems with the context

image::images/devon_guide_cobigen_5_error.png[,scaledwidth=80%]

This happens because you need to update the templates. So select Health Check option and you will see the next message 

image::images/devon_guide_cobigen_6_error.png[,scaledwidth=80%]

Click in Advance Health Check

image::images/devon_guide_cobigen_7_error.png[,scaledwidth=80%]

Now upgrade the template to constants/security_permissions and press OK. You can use the security_permissions now.
====
 
This action opens a code generator wizard, like this:

image::images/devon_guide_cobigen_8_permissions_template_selection.png[,scaledwidth=80%]

In this case you have only an option. Select Permissions Constants and press Finish. You can see now the new Permissions in /oasp4j-sample-core/src/main/java/io/oasp/gastronomy/restaurant/general/common/api/constants/PermissionConstants.java 
...
[NOTE]
====
It is possible that you can't press Finish button in CobiGen. This happens because you are using an old version of CobiGen and the wizard can't merge the class PermissionConstants. To fix it you need to delete the class PermissionConstants.java and try again. 
====