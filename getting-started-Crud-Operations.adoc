...
[NOTE]
====
You may have problems with some PermissionConstants properties
====
...
OASP proposes role-based authorization to cope with the authorization of executing use cases of an application. OASP use the JSR250 annotations, mainly @RolesAllowed, as you have seen, for authorizing method calls against the permissions defined in the annotation body.
...
Also note that the implementation does not follow the dogmatic +RESTFUL+ approach as Devon proposes a more pragmatic way to use REST. Please refer to the OASP4J Guide's  https://github.com/oasp-forge/oasp4j-wiki/wiki/guide-service-layer[service layer] chapter for more information on the subject.
...
Consequently we have to declare this new method in the table management classes in our logic layer:
...
[NOTE]
====
While auto-completing the new imports using `Ctrl-Shift-O` after adding the above methods, select `com.mysema.query.alias.Alias` as the import for the `Alias` class.
====
...
In Devon exists a special TO (Transfer Object) called ´OrderByTo` to transmit sorting parameters from client to server. This is the JSON format that the server expects when using this TO:
...
As you can see, we have added a private method to add sorting filter to our query depending on the sort params received.