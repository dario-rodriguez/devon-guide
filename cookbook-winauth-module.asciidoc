...
# The Winauth module - Authentication with Active Directory
...
Winauth module provide you a simple authentication for your Devon applications. If you want to implement this kind of authentication in a Devon project you need to follow the next steps:
...
Create a variable of the class AuthenticationManagerAD in general/configuration/WebSecurityConfig.java
...
Add the LDAP provider to the AuthenticationManagerBuilder in the init() method. 
...
=== Step 5: Configure the winauth module
...
[NOTE]
====
As you can see the property password is encrypt. You can see some information about it https://github.com/devonfw/devon/wiki/encrypting-properties[here]. Also you can put the password without encrypt by default.
==== 

== Using the UserDetailsContextMapper with AD
As is mentioned above you can implement your own UserDetailsContextMapper or use the UserDetailsContextMapper give in this wiki. If you use the last one, you need to keep in a count the next points.

=== Roler/Groups mapper

Winauth include a group mapper that give a simple tool to map the groups of the Active Directory with a roles/groups of your application. To use it you need to configure the mapping just like that:
...
Now, if you ask the server the current user of the application, you will see the user data with him groups. 

=== Service CurrentUser

If you use the basic UserDetailsContextMapper that winauth implements, you need to modify the service currentuser in the class general/service/impl/rest/SecurityRestServiceImpl.java. 
...
[NOTE]
====
You need to import the classes UserData and UserDetailsClientToAD of the winauth module.
====

# The Winauth module - Single Sign On
...
Winauth module provide you a simple SSO authentication for your Devon applications. If you want to implement this kind of authentication in a Devon project you need to follow the next steps:
...
Create a variable of the class WinauthSSO in general/configuration/WebSecurityConfig.java
...
[NOTE]
====
We need to be carefully with the service currentuser because SSO by default is not compatible with the information of the class UserDetailsClientTo. You need to adapt this class or use a customized SSO User Details (next chapter in the wiki). 
====

== Customized Winauth SSO User Details

As you can see with the last steps you have a very simple authentication and authorization with Windows credentials. In the normal scenario you want to implements your own User Details. The simple Devon project is build with a very simple dummy implementation, so we are going to implement a simple User Details for our SSO authentication.

=== Step 1 - Create customized filter

The idea is rebuild the default filter NegotiateSecurityFilter, we can create a complete new filter or, like this example, just modify some methods. In this example we are going to modify boolean setAuthentication(...), this method is called by the method void doFilter(...) (we can modify this method as we want too) when the authentication is successfully, so we are going to build here our UserDetails.
...
As you can see this example use the UsermanagementDummyImpl that is created when you create a new Devon application, feel free to customize your own filter just use the above class with a customized Usermanagement.
...
And that's all, now we have a simple SSO Authentication with a custom UserDetails and we can use the server current user by default without problems.