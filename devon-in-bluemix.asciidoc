...
A simple example below demostrates how to work with VCAP services on Devon. In this scenario, a dashDB database service is configured in the Devon application. So, you can obtain the database credentials from VCAP service.
...
As you can see, the check is applied to make sure that if VCAP_SERVICES exist. So, if it doesn't exist, you can configure other database, throw an error, etc.
...
[NOTE]
====
In the example, you are learning how to obtain the credentials of the database. If you want to know how to configure a DB2/dashDB database, you can see it link:getting-started-database-configuration#bd2[here].
====
