...
One last remark, this routes can be secured using a special service called Guards, that forbid or permit the navigation depending on the implementation of a boolean method. This will be shown in its section in link:Client-GUI-Angular-oasp4js-managing-angular-services[Angular Services].
