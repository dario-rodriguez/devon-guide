...
==== UsersDetailsService
UserDetailsService is a core interface used to load user-specific data. It has loadUserByUsername() method to find a user entity and can be overriden to provide custom implementation. For further reading follow the link http://docs.spring.io/spring-security/site/docs/current/apidocs/org/springframework/security/core/userdetails/UserDetailsService.html[here]
