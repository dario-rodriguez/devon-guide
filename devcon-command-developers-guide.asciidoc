...
As we mentioned at the begining of this chapter Devcon allows to be extended with custom modules in an external way by adding modules written in Javascript.

[NOTE]
====
You will need to have intalled Java 8 to be able to run Javascript modules.
====
...
The _commands.json_ file located in the Javascript module folder defines the elements included in the module, from the module details, as name or description, to the commands and its parameters.

If you have follow the link:Creating-a-Java-module[Creating a Java module] section you have seen that for the Java modules we use the _@CmdModuleRegistry_ annotation to register a module. In the case of the Javascript modules this is replaced by the _commands.json_ file itself so we won't have an equivalent _module registry_ keyword.
...
- _sort_: to sort modules, if sort >=0, it will be sorted by descending value. Modules which which have value <1 will be omitted from numeric sort and will be sorted alphabetically. This modules will be appended to the modules which are sorted numerically.
...
- We will use a *commands* array to enumerate all the commands of a module. Each command will be defined with the following attributes:
	* _name_: for the command name.
	* _path_: path to the _js_ file that contains the logic of the module. If is located in the same folder than the _commands.json_ file we can provide only the name of the file, without the path.
	* _description_: for the command description that will be shown to the users.
...
As part of the _comand_ object in the _commands.json_ file we can define the parameters using the following structure of attributes:
...
-	*parameters* array to group the command parameters. For each parameter we will define the following attributes:
	...
	* _sort_: see the _sort_ attribute in the previous section.
	* _inputType_: by defatult the parameters will be represented in the Devcon's graphic user interface as text boxes but, in case we want the parameter to be a drop down list, we can specify it using this _inputType_ attribute and defining some subattributes
		** _name_: "list"
...
Each command will be defined in a separate Javascript file with a name that match the `path` attribute defined in the `commands.json` file of the module.
...
2. Once we have Devcon installed we will need to create the _scripts_ fonder within the _.devcon_ directory. You can find the _.devcon_ directory at `C:\Users\{Your User}\.devcon` (if you don't find the `.devcon` directory in try looking in `D:` drive, if the directory is not there neither check your Devcon's installation).
...
==== using the GUI
To launch Devcon's GUI we must open a command line and use the `devon -g` command. After that the Devon main window should be open and we should see our new `myJSmodule` in the list of available modules. Then if we click over the module we should see the `extract` command available.
...
== Conclusion
In this section we have seen how easy can be to extend Devcon with new modules. You can either choose to add a Java module into the core of Devcon or you can achieve the same in an external way creating your own modules with Javascript (remember that you will need Java 8 to run your Javascript modules).
