...
Below code snippet depicts that, we are constructing file object supplying file path location to be downloaded.Setting this file object in ResponseBuilder and returning Response object.

If you look at the below interface, you can tell that it is a normal interface with annotations.
...
Below code explains, Iteration through each attachments,extracting headers & inputstreams and writing to the UPLOAD_FILE_SERVER. Here we are using simple file handling operations.
