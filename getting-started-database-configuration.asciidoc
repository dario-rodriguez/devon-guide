...
In this tutorial, you will see how to configure your application to connect with a real database of your choice. Devonfw gives us a simple configuration, so you just need to add the dependency of the database libraries, create a profile, set the information about your database in the profile and activate it, update the script files, if any, as per DB syntax.
...
[NOTE]
====
The http://www-01.ibm.com/support/docview.wss?uid=swg21363866[IBM Drivers] are not freely distributed, so you can't find them in Maven. You need to get in contact with IBM or just find the license in whatever IBM software product.
====