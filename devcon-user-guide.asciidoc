...
Or providing the optionale _path_ parameter
...
In order to work properly the command must be launched from within (or pointint to) a OASP4J project directory (the oasp4j project type is defined in a _devon.json_ file with parameter 'type' setted to 'oasp4j').
