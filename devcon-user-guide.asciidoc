...
With this global parameter we can ask devcon to prompt for mandatory parameters when launching a command.

To give an example we can use the _oasp4j create_ command (that creates a new server project based on _OASP4J_ model). In this case we would need to provide several mandatory parameters so the command call would look like
[source,batch]
----
...
With the global parameter *-p* Devcon gives the user the option to not define any mandatory parameter when launching the command and provide step by step all mandatory parameters after that so the usage of some commands can be way easier.

Going back to the previous example if we use the *-p* parameter
[source,batch]
----
...
As you can see with the *-p* parameter Devcon asks for each mandatory parameter related to a command and the user can provide them one on one, getting rid of the concern of knowing what parameters needs a command.
