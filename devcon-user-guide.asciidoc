...
To run Devcon using the Devcon Graphical User Interface (GUI) , we need to run below command from the command prompt:
...
If we navigate to a particular module it will show all commands available in the respective module. You can find the "Exit option", to close the application, in the first menu on the left. Once we click on any command, the corresponding command form will get displayed on the screen. See the following screen shot:
