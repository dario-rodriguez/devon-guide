...
With the global parameter *-p* Devcon gives the user the option to avoid defining any mandatory parameter when launching the command and provide step by step all mandatory parameters after that so the usage of some commands can be way easier.
