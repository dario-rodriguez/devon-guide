...
Open a console (must have the path of Cordova executable int he PATH variable, could be the Devon console or not), then proceed to add all the platforms wanted (link:https://cordova.apache.org/docs/en/latest/guide/cli/index.html#add-platforms[Add Platforms by Cordova])

[small]#Example of a project where add Android, iOS an Windows Phone#
...
----
NOTE: If run this command in a non Mac SO, a warning appears indicating that the packing and deploy of the app will be not possible
[source,bash]
----
...
This is the most basic way to have all the required files to run our application in Cordova. To see our progress in Sencha, usually use the watch command. For this scenario we need to use the build one, this option is more restrictive than the watch, and create a ZIP file with all the files required for the webapp.
[source, bash]
----
...
Once the previous command finished, open the build ZIP file and replace all the resources inside the folder `"myApp/cordova/www"`, then you have all the files updated in the cordova folder.

Once we have all the files updated, the next step is do a `"prepare/build/run"` in Cordova for the desired platform. To see more information link:https://cordova.apache.org/docs/en/latest/reference/cordova-cli/[about]
...
Instead of do a generic build, we are going to take advantage of that specific configurations.
