...
link:getting-started-download-and-install[Getting started Download and Install Devonfw distribution] 
...
[NOTE]
In case `mvn install` fails, try running the command again as it can be due to hitch in the network connection. If installation is still not successful try installing subset of the modules using `mvn --projects <module1>,<module2> --also-make install` e.g `mvn --projects modules/microservices --also-make install` to install just Microservices module and its dependencies.
...
- The microservice must have the `microservices` module as dependency in its `pom.xml` configuration to be able to use the interceptors and the generic configuration.
