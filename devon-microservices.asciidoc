...
Run the following command in the console to change to the directory to `workspaces\examples\devon`.
...
[NOTE]
In case `mvn install` fails, try running the command again as it can be due to hitch in the network connection. If the the install is still not successful try installing subset of the modules using `mvn --projects <module1>,<module2> --also-make install`.
