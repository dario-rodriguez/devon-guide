...
To generate a new microservices infrastructure application through the Devonfw archetype you only need to open a Devonfw console (_console.bat_ script) and follow the same steps described in <<Creating your First Application,getting started creating new devonfw oasp4j application>>. But, instead of using the _standard_ archetype, we must provide the special infrastructure archetype `archetype-microservice-infra`. Remember to provide your own values for _DgroupId_, _DartifactId_, _Dversion_ and _Dpackage_ parameters, Also provide the -DarchetypeVersion with latest value:
